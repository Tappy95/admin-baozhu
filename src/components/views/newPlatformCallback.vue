    ...
    methods: {
      ...
      accountList() {
        let parameterData = {
          ...
          platform:this.formInline.platform||'xiangwan', // 设置平台选项，默认值为xiangwan
          channel:this.formInline.channel||'baozhu' // 设置渠道选项，默认值为baozhu
        }
        this.$newPlatformGet('/py/allcallback/list', parameterData).then(res => {
          if ((res.statusCode+"").startsWith("2")) {
