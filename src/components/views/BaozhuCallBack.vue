        ...
        <template>
          ...
          <el-table :class="showW?'a':'u'" :data="tableData" max-height="600">
            ...
            <el-table-column fixed="right"
              ...
              <template slot-scope="scope">
                <el-button  type="success" v-if="rep &&  scope.row.status==2" plain @click="repTap(scope.row.ordernum)" size="mini">
  ...
  export default {
    ...
    methods: {
      ...
      accountList() {
        ...
        this.$fetch('/api/BZCallback/list', parameterData).then(res => {
      ...
      repTap(id) {
        this.$confirm('此操作将重发消息, 是否继续?', '提示', {
          ...
          .then(() => {
            ...
            this.$fetch('/api/BZCallback/reSend', parameterData).then(res => {
              if ((res.statusCode+"").startsWith("2")) {
                this.$message({
                  type: 'success',
                  message: '重发成功！'
