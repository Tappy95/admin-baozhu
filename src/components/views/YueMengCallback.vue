<template>
  <div class="yuemeng-wrap">
    <div class="yuemeng-inner">
      <div class="yuemeng-header">
        <h3>第三方回调/阅盟回调</h3>
        <hr />
      </div>
      <div>
        <el-form :inline="true" :model="formInline" class="demo-form-inline">
          ...
          <el-form-item label="订单号:" >
            <el-input placeholder="请输入订单号"
                      v-model="formInline.coinCallbackId"
                      clearable></el-input>
          </el-form-item>
      ...
      <div class="yuemeng-table">
        <template>
          ...
          <el-table  :data="tableData" max-height="600">
            ...
            <el-table-column min-width="170" prop="coinCallbackId" label="订单号">
            </el-table-column>
            <el-table-column min-width="120" prop="coinCount" label="奖励金币数">
              <template slot-scope="scope">
                <span>{{scope.row.coinCount |  currency}}</span>
              </template>
            </el-table-column>
            <el-table-column width="170" :formatter="dateFormat" prop="callbackTime" label="回调时间">
            </el-table-column>
            ...
            <el-table-column fixed="right"
                             label="操作" v-if="rep" width="75" >
              <template slot-scope="scope">
                <el-button  type="success" v-if="rep &&  scope.row.state==2" plain @click="repTap(scope.row.coinCallbackId)" size="mini">
                  重发
                </el-button>
              </template>
            </el-table-column>
    ...
    methods: {
      ...
      accountList() {
        let parameterData = {
          ...
          coinCallbackId:this.formInline.coinCallbackId,
          startTime:this.formInline.startTime,
          endTime:this.formInline.endTime
        }
        this.$fetch('/api/ymCallback/list', parameterData).then(res => {
      ...
      repTap(id) {
        this.$confirm('此操作将重发消息, 是否继续?', '提示', {
          ...
          .then(() => {
            ...
            this.$fetch('/api/ymCallback/reSend', parameterData).then(res => {
              if ((res.statusCode+"").startsWith("2")) {
                this.$message({
                  type: 'success',
                  message: '重发成功！'
